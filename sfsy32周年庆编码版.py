...
class RUN:
        ...
        if response.get('success') == True:
            for item in response["obj"]:
                ...
                if status == 3:
                    print(f'>【{self.taskType}】-已完成')
                    continue
                if self.taskType in ['INTEGRAL_EXCHANGE', 'CLICK_MY_SETTING', '其他任务类型']:
                    # 处理其他任务类型
                    self.handle_task(item)  # 这里可以定义一个处理任务的函数
                else:
                    print(f'暂时不支持【{self.title}】任务')
        else:
            print(f'获取任务列表失败: {response.get("errorMessage")}')

    def handle_task(self, item):
        if item['taskType'] == 'INTEGRAL_EXCHANGE':
            self.EAR_END_2023_ExchangeCard()
        elif item['taskType'] == 'CLICK_MY_SETTING':
            self.taskCode = item["taskCode"]
    ...
    def EAR_END_2023_ExchangeCard(self):
        print(f'>>>开始积分兑换年卡')
        current_time = datetime.now()
        target_time = datetime(2025, 4, 8, 19, 0)  # 根据实际活动时间设置
    ...
    def main(self):
        ...
        # 直接参与32周年庆活动
        self.participate_in_anniversary_activity()
    ...
    def participate_in_anniversary_activity(self):
        print('>>> 开始参与32周年庆活动')
        url = 'https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberNonactivity~anniversary2025ParticipateService~participate'
        
        # 构建请求数据
        data = {
            "activityCode": "ANNIVERSARY_2025",
            "userId": self.user_id,  # 使用当前用户的ID
            # 其他必要的参数
        }
        ...
        if response.get('success'):
            ...
            print(f'参与活动失败: {response.get("errorMessage")}')
...
if __name__ == '__main__':
    ...
    token = 'https%3A%2F%2Fmcs-mimp-web.sf-express.com%2Fmcs-mimp%2Fshare%2Fapp%2FshareRedirect%3Fsign%3DX9mlZlDv2F3sp3b3bBC%252BoZbMTDQgSb0FzMeeCSb0xWcQ4sgguDb0Yoa4TcE2cR1lToSY4XYmXoxgc7w0y0Vm7ireeelfWlhdcSBrimuW%2Fg%252BX6CLUJ8jVoBn1cA2Uz2cUdlzP5N6p5TQX9MPt2qwlt6bsSpgwaT4KgvPoca0cAPig8gTElL%252BR9ccLmVqKn57bcAcGh9hAR2SfIYTAABKuLvHDLRGdazZKc9vSIkBhV6DXAbrbrrdBe%252B8vvGbcW3DpzuN4rvKrPXulNTm2we6LjqJDOWVRKdqVHLxi3CSTem2TMBv7vgCjdU6JhkzjpPN%252BBSjFWDI%2FHPIKUajQvCQfug%253D%253D%26source%3DSFAPP%26bizCode%3D619'
    # 将分隔符从\n改为&
    tokens = token.split('&')
    # print(tokens)
    if len(tokens) > 0:
